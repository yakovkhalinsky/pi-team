...
```
<final_answer>
headline: classify as build task, assign builder with pathScope=src/auth

goal: add session-store fallback when Redis is unreachable

classification: build

priority: P1

owner_profile: builder

reasoning:
- change is bounded to auth session read/write paths
- no live-system operation is required; tests and typecheck are sufficient verification

context_for_owner:
- current read path in src/auth/session.ts:42 does not handle Redis errors
- tests exist in src/auth/session.test.ts but miss the failure path

next_recommendation:
- builder on src/auth with pathScope=src/auth; implement fallback and add test case; require npm test passes
