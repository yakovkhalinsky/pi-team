...
export function getOrchestratorPromptPath() {
    return resolve(promptsRoot, "orchestrator.md");
}
function getGenericWorkerPromptPath() {
    return resolve(promptsRoot, "agents/_generic-worker.md");
}
export function getWorkerPromptPath(profileName, config = DEFAULT_TEAM_CONFIG) {
...
export function loadOrchestratorPrompt() {
 ...
 */
export function loadWorkerPrompt(profileName, config = DEFAULT_TEAM_CONFIG) {
    const profile = config.profiles.find((item) => item.name === profileName);
    if (!profile) {
        throw new Error(`Unknown profile prompt contract: ${profileName}`);
    }
...
function sanitizeDescriptionSingleLine(value, fallback) {
...
function sanitizeDescriptionBlock(value, fallback) {
    const cleaned = stripControls(value).replace(/\r\n?/g, "\n").trim();
    if (!cleaned)
        return fallback;
    return cleaned.length > MAX_PROFILE_DESCRIPTION_LEN ? `${cleaned.slice(0, MAX_PROFILE_DESCRIPTION_LEN - 1)}…` : cleaned;
}
function renderGenericWorkerPrompt(profile) {
...
function buildAvailableProfilesBlock(config) {
...
export function buildOrchestratorPromptBundle(state, config = DEFAULT_TEAM_CONFIG, routingMode = "team") {
