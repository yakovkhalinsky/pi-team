...
import { DEFAULT_MODEL_SENTINEL, DEFAULT_PROMPT_SENTINEL, PROJECT_CONFIG_DIAGNOSTIC_SEVERITIES, PROJECT_CONFIG_STATUSES, TEAM_PROJECT_CONFIG_DIR, TEAM_PROJECT_CONFIG_FILE, TEAM_PROJECT_CONFIG_RELATIVE_PATH, TEAM_PROJECT_SCHEMA_VERSION, TEAM_PROJECT_SCHEMA_VERSIONS_SUPPORTED, THINKING_LEVELS, isPackagedProfileName, } from "../types.js";
import { isRecursiveOrchestratorExtensionSource } from "../safety/self-extension.js";
function clonePathScope(pathScope) {
    if (!pathScope)
        return undefined;
    return {
...
function sanitizeThinkingLevelsForSchema(scope, value) {
    ...
    for (const [profileName, rawRole] of Object.entries(value.roles)) {
        if (!isRecord(rawRole) || !("thinkingLevel" in rawRole))
            continue;
        const badValue = rawRole.thinkingLevel;
        if (THINKING_LEVELS.includes(badValue))
            continue;
...
function isPathScopeNarrowerOrEqual(candidate, baseline) {
    if (!candidate)
        return baseline === undefined;
    if (!baseline)
        return true;
    if (candidate.allowWrite && !baseline.allowWrite)
        return false;
    if (candidate.allowReadOutsideRoots && !baseline.allowReadOutsideRoots)
        return false;
    return candidate.roots.every((candidateRoot) => baseline.roots.some((baselineRoot) => isPathInsideRoot(candidateRoot, baselineRoot)));
}
function isPathInsideRoot(targetPath, root) {
...
function isLocalExtensionPathSource(source) {
...
function resolveLayerPath(layerRoot, value, fieldPath, options) {
    ...
    if (options.requireInsideLayerRoot) {
        ...
        if (realCandidate !== resolved) {
            ...
            if (!isPathInsideRoot(realCandidate, realRoot)) {
                return {
                    diagnostic: makeDiagnostic("error", "project_path_escape", `Resolved path must stay within the project root: ${value} (resolves via symlink to ${realCandidate}, outside ${realRoot})`, fieldPath),
                };
            }
        }
    }
...
function looksLikePathString(value) {
    ...
    if (trimmed.length === 0)
        return false;
    if (/\n/.test(trimmed))
        return false; // multi-line → definitely prose
    ...
    if (trimmed.startsWith("http://") || trimmed.startsWith("https://"))
        return true;
    if (trimmed.endsWith(".md") || trimmed.endsWith(".txt") || trimmed.endsWith(".prompt"))
        return true;
...
function normalizePathScope(pathScope, layerRoot, fieldPath, options) {
    if (!pathScope)
...
function normalizeExtensionSources(sources, layerRoot, fieldPath) {
    ...
    for (const [index, source] of sources.entries()) {
        ...
        if (trimmed.length === 0) {
            diagnostics.push(makeDiagnostic("error", "extension_source_empty", `Role extension source at ${entryPath} must be a non-empty string.`, entryPath));
            continue;
        }
        if (isRecursiveOrchestratorExtensionSource(trimmed, layerRoot)) {
            diagnostics.push(makeDiagnostic("error", "recursive_orchestrator_extension_forbidden", `Role extension source at ${entryPath} would load pi-agents-team recursively as a worker extension: ${trimmed}. Use a third-party provider extension instead.`, entryPath));
            continue;
        }
...
function normalizePromptPath(profile, roleConfig, layerRoot, fieldPath, options) {
    ...
    if (prompt.source === "builtin") {
        ...
        return { promptPath: profile.promptPath, diagnostics: [] };
    }
    if (!prompt.path) {
        return {
            promptPath: profile.promptPath,
    ...
    if (!existsSync(resolved.path)) {
        return {
            promptPath: profile.promptPath,
            diagnostics: [
                makeDiagnostic("warning", "project_prompt_missing", `Prompt file not readable: ${prompt.path} (${options.layerScope} ${options.layerPath}) — falling back to the built-in ${profile.name} prompt.`, `${fieldPath}.path`),
            ],
        };
    }
...
function normalizeFlatWritePolicy(write) {
    if (write === undefined)
        return undefined;
 ...
 */
export function normalizeRawRoleConfig(raw) {
    const flat = raw;
    const access = flat.access ?? {};
    const model = flat.model === undefined || flat.model === null || flat.model === DEFAULT_MODEL_SENTINEL
        ? null
        : flat.model;
    let prompt;
 ...
 */
function resolveRolePrompt(roleName, prompt, layer) {
    ...
    if (!raw || raw.trim().length === 0) {
        return {
            ...
            diagnostics: [
                makeDiagnostic("warning", "project_prompt_empty", `Prompt is empty for role "${roleName}" — using the generic worker template.`, fieldPath),
            ],
        };
    }
        ...
        try {
            if (!statSync(resolved.path).isFile()) {
                return {
                    promptPath: GENERIC_WORKER_PROMPT_SENTINEL,
                    diagnostics: [
                        makeDiagnostic("error", "project_prompt_not_a_file", `Prompt path for role "${roleName}" is not a regular file: ${raw}.`, fieldPath),
                    ],
                };
            }
        }
            ...
            return {
                promptPath: GENERIC_WORKER_PROMPT_SENTINEL,
                diagnostics: [
                    makeDiagnostic("error", "project_prompt_not_a_file", `Prompt path for role "${roleName}" could not be read: ${raw} (${error instanceof Error ? error.message : String(error)}).`, fieldPath),
                ],
            };
        }
        return { promptPath: resolved.path, diagnostics: [] };
    ...
    if (looksLikePathString(raw)) {
        return {
            promptPath: GENERIC_WORKER_PROMPT_SENTINEL,
            diagnostics: [
                makeDiagnostic("error", "project_prompt_missing", `Prompt file for role "${roleName}" was not found: ${raw}.`, fieldPath),
            ],
        };
    }
 ...
 */
function materializeRoleProfile(roleName, raw, layer) {
    ...
    const writePolicy = permissions.writePolicy ?? "read-only";
    if (writePolicy === "read-only" && resolvedPathScope.pathScope?.allowWrite) {
        diagnostics.push(makeDiagnostic("error", "read_only_scope_write_forbidden", `Role "${roleName}" is read-only but declares a writable path scope.`, `${fieldBase}.access.pathScope.allowWrite`));
    }
    const extensionMode = permissions.extensionMode ?? "worker-minimal";
    if (extensionMode === "inherit") {
        diagnostics.push(makeDiagnostic("error", "extension_mode_inherit_forbidden", `Role "${roleName}" uses extensionMode "inherit", which would let workers recursively boot the orchestrator. Use "worker-minimal" or "disable".`, `${fieldBase}.access.extensionMode`));
    }
    if (extensionMode === "disable" && resolvedExtensions.extensions.length > 0) {
        diagnostics.push(makeDiagnostic("error", "extension_mode_disable_extensions_forbidden", `Role "${roleName}" sets extensionMode "disable" but also declares access.extensions. Remove access.extensions or use "worker-minimal".`, `${fieldBase}.access.extensions`));
    }
...
export function findNearestProjectConfigPath(cwd) {
...
export function getGlobalProjectConfigPath() {
...
function parseLayer(scope, path) {
    ...
    try {
        ...
        return {
            ...
            path,
            fatalDiagnostics: [
                makeDiagnostic("error", "project_config_parse_failed", `Could not parse ${scope} ${TEAM_PROJECT_CONFIG_FILE} at ${path}: ${error instanceof Error ? error.message : String(error)}`),
            ],
        };
    }
    ...
    return {
        ...
        parsed: sanitizedJson,
        diagnostics: [],
        thinkingLevelWarnings,
    };
}
function isFatalLayerParse(value) {
    return "fatalDiagnostics" in value;
}
function makeActiveConfigFreshness(winningScope, layers, fatalScopes) {
...
export function loadActiveTeamConfig(options = { cwd: process.cwd() }) {
    ...
    const layers = [];
    const diagnostics = [];
    const thinkingLevelWarnings = [];
    const parsedLayers = [];
    const fatalScopes = new Set();
    ...
    const fatalDiagnosticsByScope = new Map();
    for (const [scope, path] of [["global", globalPath], ["project", projectPath]]) {
        if (!path)
            continue;
        ...
        const parsedAny = result.parsed;
        const layerScaffoldVersion = typeof parsedAny.scaffoldVersion === "number"
            ? parsedAny.scaffoldVersion
            : typeof parsedAny.defaultsVersion === "number"
                ? parsedAny.defaultsVersion
                : undefined;
        const rawSchemaVersion = typeof parsedAny.schemaVersion === "number"
            ? parsedAny.schemaVersion
            : typeof parsedAny.version === "number"
                ? parsedAny.version
                : undefined;
        const schemaMismatch = rawSchemaVersion === undefined ||
            !TEAM_PROJECT_SCHEMA_VERSIONS_SUPPORTED.includes(rawSchemaVersion);
        if (schemaMismatch) {
            ...
            diagnostics.push(makeDiagnostic("warning", "schema_version_mismatch", `${scope} ${TEAM_PROJECT_CONFIG_FILE} at ${path} uses schemaVersion ${rawSchemaVersion ?? "?"} (supported: ${TEAM_PROJECT_SCHEMA_VERSIONS_SUPPORTED.join(", ")}). Run /team-init ${initSubcommand} --force to regenerate with current best-practice defaults (the old file will be backed up first).`));
            layers.push({
                scope: result.scope,
                path: result.path,
    ...
    const winningScope = projectFilePresent ? "project" : "global";
    ...
    const persistedRoutingMode = winningLayer?.parsed.routingMode === "team" || winningLayer?.parsed.routingMode === "solo"
        ? winningLayer.parsed.routingMode
        : undefined;
    // Read display.cost from the winning layer; default true when absent.
    const displayCost = winningLayer?.parsed.display?.cost ?? true;
    ...
    const nonWinningFatalDiagnostics = [];
    for (const [scope, fatalDiagnostics] of fatalDiagnosticsByScope) {
        if (scope !== winningScope)
            nonWinningFatalDiagnostics.push(...fatalDiagnostics);
    }
    let profiles;
    if (!winningLayer || !winningLayer.parsed.roles || Object.keys(winningLayer.parsed.roles).length === 0) {
        ...
        const roles = winningLayer.parsed.roles;
        const application = {
    ...
    for (const layer of layers) {
        if (layer.schemaMismatch)
            continue; // already announced as a warning
...
export function formatProjectConfigDiagnostics(result) {
    if (result.diagnostics.length === 0)
        return "No project config diagnostics.";
...
export function isProjectConfigStatus(value) {
    return PROJECT_CONFIG_STATUSES.includes(value);
}
